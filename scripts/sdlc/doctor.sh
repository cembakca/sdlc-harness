...
node "$ROOT/sdlc/roster.ts" --check >/dev/null 2>&1 && ok "roster kurallari temiz" || bad "roster --check basarisiz"
...
if "$ROOT/scripts/sdlc/memory.sh" status >/dev/null 2>&1; then
  ...
  if [ "${DOCTOR_FULL:-0}" = "1" ]; then
    if MEMORY_TIMEOUT="${DOCTOR_MEMORY_TIMEOUT:-90}" "$ROOT/scripts/sdlc/memory.sh" verify >/dev/null 2>&1; then
  ...
  elif MEMORY_TIMEOUT="${DOCTOR_MEMORY_TIMEOUT:-45}" "$ROOT/scripts/sdlc/memory.sh" recall "saglik denemesi" >/dev/null 2>&1; then
    ...
    if [ -f "$ROOT/sdlc/quality-probe.json" ]; then
      ...
      if COGNEE_DATASET="$PRODUCT_DATASET" QUALITY_MIN="${QUALITY_MIN:-60}" \
         "$ROOT/scripts/sdlc/memory.sh" quality >/dev/null 2>&1; then
    ...
    warn "cognee ayakta ama OKUMA CALISMIYOR — recall yanit vermedi; hat emsal goremez.
       kanit icin: make sdlc-memory-verify · sebep icin: docker logs --tail 30 cognee-backend"
  fi
else
