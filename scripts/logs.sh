#!/usr/bin/env bash
# DEVREDEN CIKTI (D1) — logs.sh artik `make` arayuzune yonlendiriyor.
#
...
echo "scripts/logs.sh devreden cikti — karsiligi: make logs" >&2

exec make -C "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)" logs "$@"
